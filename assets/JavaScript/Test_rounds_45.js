...
cc.Class({
    ...
    Result:function(result){
        ...
        if(this.right == 3&&result){
            this.node.getChildByName("Notice").getChildByName("Words").getComponent(cc.Label).string = "恭喜你成功通过了考核！皇天不负有心人，你靠着自己的努力成功晋升为尚食，统领后宫膳食各项事宜，向皇后娘娘汇报，望卿好好把持大小诸事！";
            this.addScore(1);
        }
        else{
            if(result)
                    ...
                    .get({
                        success(res) {
                                DB.collection('TestResult').doc(res.data[0]._id).update({
                                    data:{
                                        round45:cc.sys.localStorage.getItem('level'),
                                    }
                                })
                        }
                    });
